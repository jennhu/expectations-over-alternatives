...
# Read data.
df = pd.read_csv("./human_data/pvt21.csv")
...
for i, row in df.iterrows():
    ...
    for word_idx, word in enumerate(vocab):
        ...
        sentence = row.masked_strong_sentence.replace("[MASK]", word)
        weak = row.weak_adj
        if word == weak:
    ...
    suite_path = f"./test_suites/pvt21/{scale_id}.json"
